 ...
 * Shots that agree on a gun combine. Shots that do not agree come from another gun.
 */
...
import { GAME_UNIT_M, SOURCE_TOL_DEG, SightingSolver, anchorGame, craterGame, craterZ, standZ, toMeters } from './sightings.ts';
import { BALLISTICS, WEAPONS } from './ballistics.ts';
import { costFloor, flightPath, rayMisses } from './ballisticFit.ts';
...
export interface ShotResult {
  ...
  gun?: Gun | null;
  n?: number;
  /** The Monte Carlo gun positions (meters). `steep` marks ground too steep for a gun (the worker sets it). */
  ...
  slope?: Grid;
  /** The 90th percentile distance of the Monte Carlo guns from the result (m). */
  err90?: number;
  /** The direction range from the crater (deg), as [from, to]. */
  dirRange?: [number, number];
  /** The sighting position in each clip, as the solver estimates it (meters). */
  observers: Vec3[];
  notes: string[];
  ...
  /** The crater the solve found from the sighting position (game units), with its error (m), when none was given. */
  ...
  /** The direction (deg) of the independent model (section 12.6), which fits a straight flight with gravity without the weapon. */
  independent?: { dirDeg: number; rmsDeg: number };
  /** How far (m) the solved sighting position lies from the minimap position (section 13), when there is one. */
  observerOffM?: number;
  /**
   * Each sighting against the fit (meters, like C). It holds the sighting position on its frame, the shell on the fitted
   * flight then, the seconds before the impact, and the miss (deg) along the path of the shell (signed) and across it.
   */
  ...
  /** The fitted flight (meters, like C), from the gun to the impact, each point with its time before the impact (s). */
  flight?: { tau: number; P: Vec3 }[];
}

/**
 * One gun, which combines the shots that agree on it. `single` is a gun of one shot, `combined` weighs several shots by
...
export interface ProjectResult {
  ...
  ground?: string;
  /** Where the guns can hit, from the terrain. The codes are 1 for a low arc, 2 for only the high arc, and 3 for nothing. The worker sets it. */
...
/**
 * The error (s) of a frame time, which moves a fast shell along its path in the image (section 12.5), for when the
 * marks of a clip cannot tell it (frameTiming). The value lies between the entire screen and the window capture of the
 * capture test.
 */
export const TIMESTAMP_SIGMA_S = 0.015;
/** The frame time error (s) of a sighting at a jump. It spans several frames, so its place along the path says nothing. */
const FREE_TIME_S = 0.25;
/** The least and the largest frame time error (s) that frameTiming gives. */
const TIMING_MIN_S = 0.003, TIMING_MAX_S = 0.08;
/** How far (m) the minimap puts the sighting position off the true one. The solver also uses it for a position the user typed. */
export const OBSERVER_SIGMA_M = 10;
/** How far (m) a crater the user typed or measured may be off. */
export const CRATER_SIGMA_M = 1;
/** The last part of the flight (s) that the independent model looks at. Over it, the model fits a straight flight with gravity. */
 ...
 * image there (from its neighbors), and the error of its frame time at that speed as its error along the path. The
 * sightings of a jump (`free`, where the recording skipped or repeated frames) get a frame time error of FREE_TIME_S,
 * so they count only across the path. On synthetic runs with 15 to 30 percent of such frames, that was as good as
 * leaving them out. With 15 sightings of the L52, it gave 6 / 15 m and 9 / 26 m, against 8 / 18 m and 11 / 30 m with
 * them. Also, a good frame that the check takes for a jump still helps.
 */
function prepare(data: ProjectData, shot: Shot, solver: SightingSolver, speeds: Map<Id, number>, timing: Map<Id, number>, free = new Set<Id>()): { rays: Ray[]; bad: number } {
  ...
  const rays = solved.map(({ s, ray }) => {
    const move = (speeds.get(s.id) ?? 0) * D2R * (free.has(s.id) ? FREE_TIME_S : timing.get(s.clipId) ?? TIMESTAMP_SIGMA_S);
    return { ...ray, sigmaAlong: Math.hypot(ray.sigma ?? 0, move) };
  });
  // The direction of motion at each ray points from the ray before toward the next ray, across the ray.
...
/**
 * The error of the frame times of each clip (s), from the shell itself. The shell moves smoothly, so the jitter of its
 * marks against a local fit of their neighbors, over its speed, is the error of the frame times. A frame time moves a
 * mark only along the path, but the mark error moves it both ways. Thus the jitter across the path takes the mark error
 * out of the jitter along it. Marks of a slow shell say little, so only those moving faster than 3 deg/s count. The
 * capture test measured 3 to 11 ms for a capture of the entire screen and about 27 ms for a window
 * (capture-test-plan.md). The turns of the camera cannot tell it, because the hand speeds up and slows down along the
 * turn by far more (45 to 170 ms at the 12 fps of the test clips).
 */
export function frameTiming(data: ProjectData, solver: SightingSolver): Map<Id, number> {
  const out = new Map<Id, number>(), errs = new Map<Id, { along: number[]; across: number[] }>();
  for (const shot of data.shots) {
    ...
    for (const [clip, pts] of byClip) {
      ...
      const e = errs.get(clip) ?? errs.set(clip, { along: [], across: [] }).get(clip)!;
      for (let k = 3; k < pts.length - 3; k++) {
        const nb = [k - 3, k - 2, k - 1, k + 1, k + 2, k + 3], t0 = pts[k].t, ts = nb.map((j) => pts[j].t - t0);
        // The direction as a quadratic in time, per component, gives where the mark should be and how fast it moves.
        const fit = [0, 1, 2].map((c) => quadratic(ts, nb.map((j) => pts[j].D[c])));
        if (fit.some((f) => !f)) continue;
        const v = fit.map((f) => f![1]), sp = Math.hypot(v[0], v[1], v[2]);
        if (sp * R2D < 3) continue;
        const D = pts[k].D, r = D.map((x, c) => x - fit[c]![0]), u = v.map((x) => x / sp);
        const side = [D[1] * u[2] - D[2] * u[1], D[2] * u[0] - D[0] * u[2], D[0] * u[1] - D[1] * u[0]];
        // The fit through the neighbors errs too, by their own jitter. Thus the miss has 1 + c times the variance of
        // one mark.
        const S = [0, 1, 2, 3, 4].map((p) => ts.reduce((a, t) => a + t ** p, 0));
        const c = (S[2] * S[4] - S[3] * S[3]) / (S[0] * (S[2] * S[4] - S[3] * S[3]) - S[1] * (S[1] * S[4] - S[2] * S[3]) + S[2] * (S[1] * S[3] - S[2] * S[2]));
        const k1 = 1 / (sp * Math.sqrt(1 + c));
        e.along.push((r[0] * u[0] + r[1] * u[1] + r[2] * u[2]) * k1);
        e.across.push((r[0] * side[0] + r[1] * side[1] + r[2] * side[2]) * k1);
      }
    }
  }
  const robust = (xs: number[]) => {
    const med = [...xs].sort((a, b) => a - b)[xs.length >> 1];
    return 1.4826 * xs.map((x) => Math.abs(x - med)).sort((a, b) => a - b)[xs.length >> 1];
  };
  for (const [clip, { along, across }] of errs) {
    if (along.length < 5) continue;
    const sa = robust(along), sc = robust(across);
    out.set(clip, Math.min(TIMING_MAX_S, Math.max(TIMING_MIN_S, Math.sqrt(Math.max(0, sa * sa - sc * sc)))));
  }
  return out;
}
...
/**
 * The angular speed of the shell (deg/s) at each sighting. It is the speed of the step to the sighting, or of the step
 * from it for the first sighting of a run.
 */
...
/** A shot the solver can place, with its anchor (the crater or the sighting position) and the sighting position as a prior. */
interface Setup {
  C: Vec3;
  /** True when the crater is not known. Then the rays start at the sighting position, and the solve finds the crater. */
  findCrater: boolean;
  /** The sighting position from the minimap, as a shift from the crater (m). */
  prior?: ShiftPrior;
}
...
function setup(shot: Shot, heights?: Heights): Setup | null {
  ...
  const C = toMeters(at, craterZ(shot, heights));
  const crater = craterGame(shot), obs = shot.observer[shot.clipId];
  const O = value(obs);
  if (!crater || !O) return { C, findCrater: !crater };
  // When both are known, the minimap position pulls the solved sighting position, with the errors of both.
...
/** Solves the shots of a project with one weapon, without Monte Carlo runs, and returns the RMS fit error over all shots. */
...
function pickWeapon(data: ProjectData, heights?: Heights, ground?: GroundAt): ProjectResult['weapon'] {
  ...
  // A fit error of a few hundredths of a degree is noise, so the ratio counts from there.
  ...
  const auto: Detected<Weapon> = best
    ...
    : { conf: 0, reason: 'the solver cannot solve any shot yet' };
...
export function solveProject(data: ProjectData, rng: Rng = Math.random, runs = MC_RUNS, heights?: Heights, ground?: GroundAt): ProjectResult {
  ...
  for (const shot of data.shots) {
    ...
    if (!su) { r.error = 'Enter the crater X and Y in Coordinates, or the sighting position.'; continue; }
    const clip = shot.clipId!;
    const jumped = new Set(jumps.filter((f) => f.shotId === shot.id).flatMap((f) => [f.from, f.to]));
    const prep = prepare(data, shot, exact, speeds, timing, jumped);
    ...
    // Without a crater, the rays start at the sighting position. The crater is that position minus the shift the fit
    // found, and the whole solution moves with it.
    const move = (s: number[]): [number, number] => (su.findCrater ? [-s[0], -s[1]] : [0, 0]);
    ...
    const zObs = standZ(shot, heights);
    ...
    r.flight = flightPath(opt.ballistics, sol.fit, su.C, opt.zGun).map((q) => ({ tau: q.tau, P: at(q.P) }));

    const shifts: number[][] = [];
    for (const J of jittered) {
      const m = solveShot(prepare(data, shot, J, speeds, timing, jumped).rays, su.C, { ...opt, near: { th: sol.fit.th, e: sol.fit.e } });
    ...
    if (su.findCrater) {
      // the spread of the shift, and the error of the minimap position the crater depends on
    ...
    // The second opinion on the direction uses the last second of the flight, or the 6 rays nearest the impact.
    ...
    if (su.prior) {
      // The minimap position against the sighting position from the rays alone (sections 10.3 and 13). The prior
      // pulls the fit toward the minimap position, so that fit would hide a disagreement.
      ...
      r.observerOffM = d;
      if (d > 3 * su.prior.sigma && d > OBSERVER_SIGMA_M) r.notes.push(`The sightings put the sighting position ${d.toFixed(0)} m from the one of the minimap and the crater. Check the crater and the impact time.`);
    }

    // If two directions fit about equally well, the user must say which one is right (section 12.3).
...
// runs, two estimates agree within a fixed distance.
 ...
 * less well along it. Thus shots from different directions fix each other, and shots from one direction still add
...
function groupGuns(shots: ShotResult[]): GunGroup[] {
  ...
  return groups.map((g) => {
    ...
    // The accuracy comes from the same combination of each Monte Carlo run.
