...
/** What a sighting still lacks: the shell, and its own camera data unless it copies the previous camera. */
export function lacks(s: Sighting): string[] {
  const out: string[] = [];
  if (!s.shell) out.push('shell');
  if (!s.sameCameraAsPrevious && !s.edges.length) out.push('vertical edge');
  if (!s.sameCameraAsPrevious && s.headingDeg == null) out.push('compass heading');
  return out;
}
...
export function missing(p: ProjectData, clips: Pick<ClipMeta, 'id' | 'name'>[]): Missing {
  ...
  for (const shot of usedShots) {
    ...
    const noImpact = new Set(list.filter((s) => shot.impactTimeS[s.clipId] == null).map((s) => s.clipId)).size;
