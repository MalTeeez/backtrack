  ...
  <div class="card-body flex flex-col gap-3">
    ...
    {:else if r.fit}
      ...
      <dl class="dl">
        ...
        {#if r.crater}<dt>Crater</dt><dd title="Solved from the sighting position and the end of the flight">X {r.crater.x.toFixed(2)}  Y {r.crater.y.toFixed(2)} <span class="text-muted">(+/-{f0(r.crater.sigmaM)} m, solved)</span></dd>{/if}
        <dt>Sighting position</dt><dd>{#each Object.values(r.fit.shifts) as [dx, dy], i}{i ? ', ' : ''}{f0(Math.hypot(dx, dy))} m from the crater{/each}</dd>
        {#if r.independent}<dt title="A straight flight with gravity over the last second, without the weapon table. It gives a second opinion on the direction (automation plan section 12.6).">Straight flight</dt><dd>{r.independent.dirDeg.toFixed(1)} deg <span class="text-muted">({(((r.independent.dirDeg - r.fit.th + 540) % 360) - 180).toFixed(1)} deg from the solver)</span></dd>{/if}
        {#if r.observerOffM != null}<dt title="How far the sightings put the sighting position from the minimap position. Over many clips this shows a bias of the solver (automation plan section 13).">Minimap offset</dt><dd>{f0(r.observerOffM)} m</dd>{/if}
